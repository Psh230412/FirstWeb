			...
			<section class="movie-view">
				<div class="movie-view-container">
					...
					<c:forEach var="entry" items="${postersMap}">
						<div class="movie-container">
							<div class="poster-container">
								<img src="data:image/jpeg;base64,${entry.value}"
									class="moviePoster" />
							</div>
							<div class="movies-list-name"></div>
										...
										<c:forEach var="imgData" items="${locationObject.imageData}"
											...
											<div class="item"
												...
												<div class="description">
													<p class="address">
														${locationObject.addressData[status.index]}</p>
												</div>
											</div>
										</c:forEach>
									</c:forEach>
					...
					<form action="./selectpath" method="POST" id="form">
						...
						<input type="hidden" id="selectedLocationNos"
							name="selectedLocationNos" /> <input type="submit"
							class="btn btn-primary nextPage" value="경로 확인하기" disabled="true">
					</form>
				</div>
			</section>
			<!-- 
