...
public class User_Choice_Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	ObjectMapper objectMapper = new ObjectMapper();
	User_Choice_DAO choice_DAO = new User_Choice_DAO();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {		
	        ...
	        if(loadedMovieIds.isEmpty()) {
	        	...
	        	sql = "SELECT * FROM movie WHERE movie_no NOT IN (" + String.join(",", Collections.nCopies(loadedMovieIds.size(), "?")) + ") ORDER BY RAND() LIMIT 12";
	        	pstmt = conn.prepareStatement(sql);
